...
class MultiSourcePostVideoAttachmentSQLiteTable {
    ...
    func create(databaseConnection: OpaquePointer) throws {
        do {
            let statement =
                """
                CREATE TABLE
                multi_source_post_video_attachment(
                    id TEXT PRIMARY KEY,
                    post_id TEXT,
    ...
    func insert(databaseConnection: OpaquePointer, id: String, postId: String, orderNumber: Int64, remoteURL: String) throws {
        do {
            let statement =
                """
                INSERT INTO multi_source_post_video_attachment(id, post_id, order_number, remote_url)
                VALUES (?, ?, ?, ?);
                """
            let preparedStatement = try sqlite3PrepareV2(databaseConnection, statement)
    ...
    func select(databaseConnection: OpaquePointer, wherePostId postId: String) throws -> [Row] {
        do {
            let statement =
                """
                SELECT id, post_id, order_number, remote_url FROM multi_source_post_video_attachment
                WHERE post_id = ?;
                """
            let preparedStatement = try sqlite3PrepareV2(databaseConnection, statement)
